...
# 解决方案
# 默认情况下 re 模块已经对一些 Unicode字符类有了基本的支持。
# 比如， \\d 已经匹配任意的 unicode数字字符了：

import re

num = re.compile(r'\d+')
# ASCII digits
print(num.match('1234'))  # <_sre.SRE_Match object; span=(0, 4), match='1234'>
# Arabic digits
print(num.match('\u0661\u0662\u0663'))  # <_sre.SRE_Match object; span=(0, 3), match='١٢٣'>

# 如果你想在模式中包含指定的 Unicode字符，你可以使用 Unicode字符对应的转义序列(比如 \uFFF 或者 \UFFFFFFF )。
...
# 讨论
# 混合使用 Unicode和正则表达式通常会让你抓狂。
# 如果你真的打算这样做的话，最好考虑下安装第三方正则式库，
# 它们会为 Unicode的大小写转换和其他大量有趣特性提供全面的支持，包括模糊匹配。
