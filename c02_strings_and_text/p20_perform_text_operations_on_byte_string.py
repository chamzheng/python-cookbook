...
# 最后需要注意的是，使用字节字符串可能会改变一些操作的语义，
# 特别是那些跟文件系统有关的操作。 比如，如果你使用一个编码为字节的文件名，
# 而不是一个普通的文本字符串，会禁用文件名的编码/解码。比如：
...
# 尽管操作字节字符串确实会比文本更加高效(因为处理文本固有的 Unicode相关开销)。
# 这样做通常会导致非常杂乱的代码。你会经常发现字节字符串并不能和 Python的其他部分工作的很好，
# 并且你还得手动处理所有的编码/解码操作。
