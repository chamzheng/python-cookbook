...
# 为了演示，假设你正在解析形如 3 + 4 * 5 的表达式。 这个表达式先要通过使用2.18节中介绍的技术分解为一组令牌流。
# 结果可能是像下列这样的令牌序列： NUM + NUM * NUM
...
# 第一个输入令牌是NUM，因此替换首先会匹配那个部分。 一旦匹配成功，就会进入下一个令牌+，以此类推。
...
class ExpressionEvaluator:
    ...
    def parse(self, text):
        ...
        self._advance() # Load first lookhead token
        return self.expr()
